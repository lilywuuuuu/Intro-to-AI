...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        # Returns num evenly spaced samples, calculated over the interval [lower, upper] without the upper point.
        bins = np.linspace(lower_bound, upper_bound, num_bins, endpoint=False) 
        bins = np.delete(bins, 0) # Delete the lower point --> num-1 evenly spaced samples. 
    ...
    def discretize_value(self, value, bins):
        ...
        return np.digitize(value, bins, right=False) # bins[i-1] ≤ value < bins[i], return i
    ...
    def discretize_observation(self, observation):
        ...
        # state = [0~6, 0~6, 0~6, 0~6] because there are 7 bins for each observation value
        state = [0, 0, 0, 0]

        for i in range(len(observation)):
            # discretize each observation value accordingly to put each value in one of the 7 bins
            state[i] = self.discretize_value(observation[i], self.bins[i])
    ...
    def choose_action(self, state):
        ...
        r = np.random.uniform(0,1) # choose a random r from 0~1
        if(r > self.epsilon): # choose best action from the current state
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        # max_Qopt(s', a')
        if done: max_next_Qopt = 0
        else: max_next_Qopt = np.max(self.qtable[next_state[0], next_state[1], next_state[2], next_state[3]])

        Qopt_old = self.qtable[state[0], state[1], state[2], state[3], action]
        # Qopt(s, a) = (1-eta) * Qopt(s, a) + eta * (reward + gamma * max_Qopt(s', a'))
        Qopt_new = (1 - self.learning_rate) * Qopt_old + self.learning_rate * (reward + self.gamma * max_next_Qopt)
        self.qtable[state[0], state[1], state[2], state[3], action] = Qopt_new # update qtable to new Qopt
    ...
    def check_max_Q(self):
        ...
        state = self.env.reset() # reinitialize state 
        state = self.discretize_observation(state) # discretize the initial state values

        return np.max(self.qtable[state[0], state[1], state[2], state[3]]) # return the max Q value of the inital state
