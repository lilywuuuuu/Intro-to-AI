...
class Agent():
    ...
    def learn(self):
        ...
        m = torch.LongTensor([0]*32)
        eval = self.evaluate_net(state).gather(1, action.unsqueeze(1)) + m
    ...
    def check_max_Q(self):
        ...
        x = torch.unsqueeze(torch.tensor(state, dtype=torch.float), 0)
        action_values = self.evaluate_net(x) # evaluate state to get Q values

        return torch.max(action_values).item() # return max Q value
