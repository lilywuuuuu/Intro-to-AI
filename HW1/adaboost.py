...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        hj(xi) = 1 if fj(xi) < 0
               = 0 if fj(xi) ≥ 0 
        hj(xi) = |hj(xi) - label(xi)|
        ej     = Σi (wi * hj(xi))
        errorIndex: the index of min error
        bestError: the min error value
        bestClf: the classifer which has feature of the lowest error value
        '''
        # h = np.abs(np.where(featureVals < 0, 1, 0) - labels)
        ''' 
        bonus: method 2 --> sigmoid function
        hj(xi) = 1/(1+e^(-xi)) 
        hj(xi) = |hj(xi) - label(xi)|
        ej     = Σi (wi * hj(xi))
        errorIndex: the index of min error
        ...
        bestClf: the classifer which has feature of the lowest error value
        '''
        h = np.abs(1/(1 + np.exp(featureVals)) - labels)
        e = np.sum(np.multiply(weights, h), axis=1)
        errorIndex = np.argmin(e)
        bestError = e[errorIndex]
        bestClf = WeakClassifier(features[errorIndex])
